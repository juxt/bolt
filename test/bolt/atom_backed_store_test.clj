...
(defn fn-validation-fixture [f]
  (s/with-fn-validation
    (f)))

(use-fixtures :each fn-validation-fixture)
...
(deftest non-expiry-tests
  (testing "create token"
    (let [tokens (atom {})
      ...
      (is (= {:a "A" :bolt/token-id "123"} (get-token-by-id component "123")))))
  ...
  (testing "renew token should return the new token"
    (let [tokens (atom {})
      ...
      (let [token (renew-token! component "123")]
        (is (= {:a "A" :bolt/token-id "123"} token))))))
